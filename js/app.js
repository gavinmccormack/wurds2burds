      ...
      var Boid = function () {
        ...
        _acceleration, _width = 100, _height = 100, _depth = 100, _goal, _neighborhoodRadius = 50,
        _maxSpeed = 0.001, _maxSteerForce = 0.1, _avoidWalls = false;
        ...
        for ( var i = wordGuide.length-1; i > 0; i-- ) {
            for(var inner = wordGuide[i].length-1; inner > 0; inner--){
              if ( wordGuide[i][inner] == 1) {
                ...
                bird = birds[ cn ] = new THREE.Mesh( new Bird(), new THREE.MeshBasicMaterial( { color:Math.random() * 0xffffff, side: THREE.DoubleSide } ) );
